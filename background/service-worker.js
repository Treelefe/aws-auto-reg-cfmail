...
import { MailClient } from '../lib/mail-api.js';
...
async function runSessionRegistration(session) {
  try {
    ...
    // 步骤 2: 创建临时邮箱（使用 API 锁）
    updateSession(session.id, { step: '创建临时邮箱...' });

    // 获取邮箱配置
    const config = await getMailConfig();
    session.mailClient = new MailClient({
      baseUrl: `https://${config.workerDomain}`,
      domain: config.emailDomain,
      adminPassword: config.adminPassword
    });

    // 使用姓名生成仿真邮箱前缀，例如：john.smith2024, johnsmith24
    const emailPrefix = generateEmailPrefix(firstName, lastName);

    const email = await withApiLock(() => session.mailClient.createInbox({
      prefix: emailPrefix
    }));
    session.email = email;
    session.mailAccessKey = session.mailClient.accessKey;
...
async function getVerificationCode(session) {
  if (!session || !session.mailClient) {
    return { success: false, error: '邮箱客户端未初始化' };
  }

  try {
    const code = await session.mailClient.waitForVerificationCode(120000, 3000);
    console.log(`[Session ${session.id}] 获取到验证码:`, code);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.type) {
    ...
    case 'UPDATE_MAIL_CONFIG':
      // 更新邮箱配置缓存
      if (message.config) {
        mailConfig = message.config;
        console.log('[Service Worker] 邮箱配置已更新:', mailConfig);
      }
      sendResponse({ success: true });
      break;
