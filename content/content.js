...
(function() {
  ...
  async function handleVerifyPage() {
    ...
    // 扩展选择器匹配更多 AWS 验证码输入框
    const codeInput = $(
      'input[placeholder*="位数"], ' +
      'input[placeholder*="位數"], ' +
    ...
    if (!codeInput) {
      console.log('[Content Script] 找不到验证码输入框，尝试查找所有 input');
      // 最后尝试：查找页面上的第一个文本输入框
      const allInputs = document.querySelectorAll('input[type="text"], input:not([type])');
      console.log('[Content Script] 页面上的输入框数量:', allInputs.length);
      for (const inp of allInputs) {
        console.log('[Content Script] 输入框:', inp.name, inp.id, inp.placeholder, inp.className);
      }
      return false;
    }

    console.log('[Content Script] 找到验证码输入框:', codeInput.name, codeInput.id);
