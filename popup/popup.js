...
// 邮箱服务配置元素
const mailWorkerDomainInput = document.getElementById('mail-worker-domain');
const mailEmailDomainInput = document.getElementById('mail-email-domain');
const mailAdminPasswordInput = document.getElementById('mail-admin-password');
const toggleMailConfigBtn = document.getElementById('toggle-mail-config');
const mailConfigFields = document.getElementById('mail-config-fields');
const saveMailConfigBtn = document.getElementById('save-mail-config');
const mailConfigStatus = document.getElementById('mail-config-status');
...
async function syncToKiro(id) {
  ...
  try {
    ...
    const command = `mkdir -p ~/.aws/sso/cache && cat > ~/.aws/sso/cache/kiro-auth-token.json << 'EOF'
${authToken}
EOF
...
async function loadMailConfig() {
  try {
    ...
    const config = result.mailConfig || DEFAULT_MAIL_CONFIG;

    mailWorkerDomainInput.value = config.workerDomain || '';
    mailEmailDomainInput.value = config.emailDomain || '';
    mailAdminPasswordInput.value = config.adminPassword || '';

    console.log('[Popup] 邮箱配置已加载');
  } catch (error) {
    console.error('[Popup] 加载邮箱配置失败:', error);
  }
}
...
async function saveMailConfig() {
  const config = {
    workerDomain: mailWorkerDomainInput.value.trim(),
    emailDomain: mailEmailDomainInput.value.trim(),
    adminPassword: mailAdminPasswordInput.value
  };

  if (!config.workerDomain || !config.emailDomain) {
    alert('Worker 域名和邮箱域名不能为空');
    return;
  }
  ...
  try {
    ...
    await chrome.runtime.sendMessage({ type: 'UPDATE_MAIL_CONFIG', config });

    mailConfigStatus.textContent = '已保存';
    setTimeout(() => {
      mailConfigStatus.textContent = '';
    }, 2000);

    console.log('[Popup] 邮箱配置已保存:', config);
  } catch (error) {
    console.error('[Popup] 保存邮箱配置失败:', error);
...
function toggleMailConfig() {
  const isHidden = mailConfigFields.style.display === 'none';
  mailConfigFields.style.display = isHidden ? 'flex' : 'none';
  toggleMailConfigBtn.textContent = isHidden ? '收起' : '展开';
}
...
async function init() {
  ...
  // 邮箱配置事件
  toggleMailConfigBtn.addEventListener('click', toggleMailConfig);
